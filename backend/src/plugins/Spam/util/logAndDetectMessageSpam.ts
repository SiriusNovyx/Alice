    ...
    async () => {
      ...
      if (recentActionsCount > spamConfig.count) {
        ...
        if (spamConfig.kick && member && !muteResult) {
          try {
            const modActionsPlugin = pluginData.getPlugin(ModActionsPlugin);
            await modActionsPlugin.kickMember(
              member,
        ...
        // Ban the user if enabled (overrides kick)
        if (spamConfig.ban && member) {
          try {
            await pluginData.guild.bans.create(member.id, {
              reason: "Automatic spam detection",
              deleteMessageSeconds: (spamConfig.ban_delete_message_days ?? 0) * 86400,
            });
            const casesPlugin2 = pluginData.getPlugin(CasesPlugin);
            casesPlugin2.createCase({
              userId: member.id,
              modId: pluginData.client.user!.id,
              type: CaseTypes.Ban,
              reason: "Automatic spam detection",
