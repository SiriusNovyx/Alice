The project is called Alice. It's a Discord bot that uses Discord.js. The bot is built on the Vety framework (formerly called Knub).
...
Alice's functionality is split into plugins, which are located in the `src/plugins` directory.
Each plugin has its own directory, with a `types.ts` for config types, `docs.ts` for a `AlicePluginDocs` structure, and the plugin's main file.
